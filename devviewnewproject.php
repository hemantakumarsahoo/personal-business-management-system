...
if(isset($_POST["assign"]))
 {
   ...
   $adate=date('d/m/y');
   include "config.php";
  $sql="update project_assigned set remarks='".$_POST['remarks']."',progressstatus='".$_POST['pstatus']."' where projectid='".$pid."'";
...
<input type="hidden" value="<?php echo $devid;?>" name="devid" />
    ...
    <label for="sq">(Update Remarks & Progress Status)</label>
   
  </fieldset>
  <fieldset class="form-group">
    <label for="sq">Remarks</label>
  ...
  <fieldset class="form-group">
    <label for="sq">Progress Status(in %)</label>
   <input type="text" class="form-control" name="pstatus" value="<?php echo $arr['progressstatus']; ?>" />
  </fieldset>
  
  <button type="submit" class="btn btn-primary" name="assign">Update It</button>
  
  <a href="devnewproject.php"> <button type="button" class="btn btn-primary">View Another Project</button></a>
  <?php 
			if(isset($msg))
			{
