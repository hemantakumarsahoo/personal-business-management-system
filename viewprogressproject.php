...
if(isset($_POST["assign"]))
 {
  ...
  $sql="update project_assigned set progressstatus='100%' where projectid='".$pid."'";
  
  $i=mysqli_query($con,$sql);
    if($i)
	{
		mysqli_query($con,"update client_project set status='completed' where pid='".$pid."'");
...
<input type="hidden" value="<?php echo $devid;?>" name="devid" />
    ...
    <label for="sq">(Completed & Delivered To Client)</label>
   
  </fieldset>
  
  <button type="submit" class="btn btn-primary" name="assign">Complete It</button>
  
  <a href="progressproject.php"> <button type="button" class="btn btn-primary">View Another Project</button></a>
  <?php 
			if(isset($msg))
			{
